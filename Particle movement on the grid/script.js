(() => {
  ...
  class Dot{
    constructor(){
      this.pos = {x : cx, y : cy}
      this.dir = cfg.dirsCont ===6 ? (Math.random() * 3 | 0) * 2 : Math.random() * cfg.dirsCont | 0
      this.step = 0
    }
    ...
    changeDir(){
      if(this.step % cfg.stepsToTurn === 0){
        this.dir = Math.random() > .5 ? (this.dir + 1) % cfg.dirsCont : (this.dir + cfg.dirsCont - 1) % cfg.dirsCont
      }
    }
  ...
  let dirsList = []
  function createDirs() {
    for(let i = 0; i < 360; i += 360 / cfg.dirsCont){
